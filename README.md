...
Demo package for Fractal task creation
...
```console
# Install only fractal_tasks_demo:
python -m pip install -e .
# Install both fractal_tasks_demo and development dependencies (e.g. pytest):
...
```console
python src/fractal_tasks_demo/dev/create_manifest.py
git add src/fractal_tasks_demo/__FRACTAL_MANIFEST__.json
git commit -m'Update `__FRACTAL_MANIFEST__.json`'
git push origin main
```

5. If you add a new task, you should also add a new item to the `TASK_LIST`
list, in `src/fractal_tasks_demo/dev/task_list.py`. Here is an example:
```python
